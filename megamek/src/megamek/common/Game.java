...
public class Game implements Serializable {
 ...
 // TODO: Correctly implement "Captured" Entities
    ...
    private void updatePlayersRating() {
        ...
        if (getVictoryPlayerId() == Player.PLAYER_NONE && getVictoryTeam() == Player.TEAM_NONE) {
            return;
        }

        for (Enumeration<Player> i = getPlayers(); i.hasMoreElements(); ) {
            final Player player = i.nextElement();
            if (getVictoryPlayerId() != Player.PLAYER_NONE && player.getId() == getVictoryPlayerId()) {
                winners.add(getPlayer(getVictoryPlayerId()));
            }
            else if (getVictoryTeam() != Player.TEAM_NONE && player.getTeam() == getVictoryTeam()) {
                winners.add(player);
            }
            else {
                losers.add(player);
